...
#[no_mangle]
pub unsafe extern "C" fn embThread_findNearestColor(
    mut color: EmbColor,
    mut colors: *mut EmbThreadList,
) -> i32 {
    ...
    'loop1: loop {
        ...
        dist = ((deltaRed * deltaRed) as (f64) + (deltaBlue * deltaBlue) as (f64)
            + (deltaGreen * deltaGreen) as (f64))
            .sqrt();
...
#[no_mangle]
pub unsafe extern "C" fn embThread_getRandom() -> EmbThread {
...
#[no_mangle]
pub unsafe extern "C" fn embThreadList_create(mut data: EmbThread) -> *mut EmbThreadList {
    let mut heapThreadList: *mut EmbThreadList =
        libc::malloc(::std::mem::size_of::<EmbThreadList>()) as (*mut EmbThreadList);
...
#[no_mangle]
pub unsafe extern "C" fn embThreadList_add(
    mut pointer: *mut EmbThreadList,
    mut data: EmbThread,
) -> *mut EmbThreadList {
    if pointer.is_null() {
        embLog_error!("emb-thread.c embThreadList_add(), pointer argument is null\n");
        0i32 as (*mut EmbThreadList)
    } else if !(*pointer).next.is_null() {
        embLog_error!("emb-thread.c embThreadList_add(), pointer->next should be null\n");
        0i32 as (*mut EmbThreadList)
    } else {
        (*pointer).next = libc::malloc(::std::mem::size_of::<EmbThreadList>())
...
#[no_mangle]
pub unsafe extern "C" fn embThreadList_getAt(
    mut pointer: *mut EmbThreadList,
    mut num: i32,
) -> EmbThread {
    let mut i: i32 = 0i32;
...
#[no_mangle]
pub unsafe extern "C" fn embThreadList_count(mut pointer: *mut EmbThreadList) -> i32 {
    let mut i: i32 = 1i32;
    if pointer.is_null() {
        0i32
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn embThreadList_empty(mut pointer: *mut EmbThreadList) -> i32 {
    if pointer.is_null() {
        1i32
    } else {
        0i32
...
#[no_mangle]
pub unsafe extern "C" fn embThreadList_free(mut pointer: *mut EmbThreadList) {
    let mut tempPointer: *mut EmbThreadList = pointer;
    let mut nextPointer: *mut EmbThreadList = 0i32 as (*mut EmbThreadList);
    'loop1: loop {
